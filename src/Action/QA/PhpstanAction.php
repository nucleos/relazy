...
final class PhpstanAction extends BaseAction
{
    ...
    {
        $this->command          = $command          ?? 'vendor/bin/phpstan';
        $this->parameter        = $parameter        ?? 'analyse --no-interaction';
    ...
    public function execute(Context $context, Console $console): void
    {
        ...
        if ($process->getExitCode() !== $this->expectedExitCode) {
            throw new CommandException('phpstan failed');
        }
    }
}
